...
Future<T?> showAppDialog<T>(
  ...
  Color barrierColor = const Color(0x66252525),
...
class AppDialog extends StatelessWidget {
  ...
  Widget _buildDialogContainer() {
    return AnimatedContainer(
          ...
          height: size?.height,
          width: size?.width,
          constraints: BoxConstraints(maxHeight: size?.height ?? 626, maxWidth: size?.width ?? 328, minHeight: 20),
          decoration: BoxDecoration(color: backgroundColor, borderRadius: BorderRadius.circular(12.0)),
          padding: contentPadding ?? const EdgeInsets.symmetric(vertical: 36, horizontal: 30),
          child: child,
        )
        .animate()
        .scaleXY(begin: 0.8, alignment: Alignment.bottomCenter, duration: 500.inMs, curve: KCurves.defaultIosSpring)
        .slideY(begin: 0.1, end: 0, duration: 500.inMs, curve: KCurves.defaultIosSpring);
  }
}
