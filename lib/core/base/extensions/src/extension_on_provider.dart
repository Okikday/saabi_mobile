...
extension RefExtensions on Ref {
  void emptyListenMany(List<ProviderListenable> providers, {bool fireImmediately = false}) {
    for (final p in providers) {
      listen(p, (_, _) {}, fireImmediately: fireImmediately);
    }
  }

  void emptyWatchMany(List<ProviderListenable> providers) {
    for (final p in providers) {
      watch(p);
    }
  }
